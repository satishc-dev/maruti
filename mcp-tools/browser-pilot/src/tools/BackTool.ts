...
export class BackTool extends BaseTool {
  public readonly name = "browser_back";
  public readonly description = "Navigate back in the browser history";
  ...
  protected async runImpl(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    return this.textResult(`Navigated back to: ${url} (title: "${title}")`);
  }
}
