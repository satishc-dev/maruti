...
[`MEMORY-SCHEMA.md`](../../../MEMORY-SCHEMA.md). Follow it exactly. Use
`TodoWrite` to track phases within any mode.
...
| `/project-lead migrate [--apply]` | **migrate** — dry-run or apply OKF migration |
| `/project-lead status` | **status** — report the funnel |
| `/project-lead sync` | **sync** — reconcile board with docs + issues/PRs |
| `/project-lead lint` | **lint** — advisory health-check and OKF conformance report |
...
tell the user to run `/project-lead bootstrap` first.
...
5. **Team availability.** `pm-team` and `dev-team` are declared **dependencies**
   ...
   they actually resolve. If they do not — e.g. a local-checkout install that
   bypassed the marketplace — guide the user to add them (`/plugin install
   pm-team@maruti`, `/plugin install dev-team@maruti`); you cannot install them
   silently.
...
- Never invoke `/pm-team` for a requirement whose `lifecycle` is not `approved`.
