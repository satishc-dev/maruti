...
## 5. Create six OKF document bundles and templates
1. Ensure bundle roots exist: `.project-memory/index.md`, `docs/requirements/index.md`, `docs/research/index.md`, `docs/ux/index.md`, `docs/specs/index.md`, `docs/architecture/index.md`.
2. Each bundle-root `index.md` has only:
   ```yaml
   ---
   okf_version: "0.2"
   ---
   ```
3. Create missing template files only.
...
5. `docs/research/_brief-template.md` frontmatter: `type: Research Brief`, `title`, `description`, `owner: team:research-team`, `req`, `topic`, `commissioned_by: team:project-lead`, `commissioned_question`, `scope: { in: [], out: [] }`, `constraints: []`, `good_answer: []`, `generated`, `status: draft`.
6. `docs/research/_finding-template.md` frontmatter: `type: Research Finding`, `title`, `description`, `owner: team:research-team`, `req`, `topic`, `brief`, `question`, `confidence: medium`, `generated`, `status: draft`, `sources: []`.
7. `docs/ux/_template.md` frontmatter: `type: UX Brief`, `title`, `description`, `owner: team:ux-team`, `req: REQ-NNN`, `feature`, `surface: user-visible`, `research: []`, `status: draft`, `generated`.
8. `docs/specs/_template.md` frontmatter: `type: Feature Spec`, `title`, `description`, `owner: team:pm-team`, `req: REQ-NNN`, `feature`, `ux: []`, `research: []`, `status: draft`, `generated`.
9. `docs/architecture/_solution-template.md` frontmatter: `type: Solution Architecture`, `title`, `description`, `owner: team:architect-team`, `req: REQ-NNN`, `specs: []`, `research: []`, `adrs: []`, `principles: []`, `status: draft`, `generated`.
10. `docs/architecture/decisions/_template.md` frontmatter: `type: Decision`, `title`, `description`, `owner: team:architect-team`, `adr: ADR-NNN`, `req`, `status: draft`, `generated`.
11. `docs/architecture/principles/_template.md` frontmatter: `type: Development Principle`, `title`, `description`, `owner: team:architect-team`, `req`, `applies_to: []`, `enforcement: must`, `source_decisions: []`, `status: draft`, `generated`.
12. Idempotency predicate: every root and template exists; existing files are not overwritten.
...
4. Idempotency predicate: one exact-title Project is selected and its number, URL and `PVT_...` id are known.
...
3. Ensure option names: Intake, In Review, Ready for Discovery, In Discovery, In Spec, In Architecture, Ready for Dev, In Dev, In Acceptance, Done, Parked, Blocked.
4. If a field mutation is unavailable, stop with manual action rather than guessing ids.
5. Idempotency predicate: field id and every option id are known.
...
7. Idempotency predicate: project-link records the selected path and fallback label exists when selected.
...
1. Run:
   ...
   ```
2. Create `workstream:ws<k>` labels later when concrete workstreams exist:
   ```bash
   gh label create "workstream:ws1" --repo <owner>/<repo> --description "Assigned delivery workstream ws1" --color fbca04 || true
   ```
3. Idempotency predicate: required repository labels exist.
...
## 13. Seed architecture principles placeholder
1. Create `docs/architecture/principles/README.md` only if missing and no real principle exists.
2. Frontmatter:
   ```yaml
   ---
   type: Development Principle
   title: Development principles placeholder
   description: Placeholder noting that Architect-Team populates binding development principles after solution architecture exists.
   owner: team:architect-team
   generated: { by: software-team-project-lead/0.1.0, at: <ISO-8601 timestamp> }
   status: draft
   ---
   ```
3. Body: Architect-Team populates this directory after a solution architecture exists; principles are binding on Dev-Team once written.
4. Idempotency predicate: placeholder exists, or real principles exist and placeholder is skipped.
## 14. Normalize log and append Bootstrap entry
...
2. Append:
   ```markdown
   - **Bootstrap**: [<ISO-8601 timestamp>] St-Project-Lead — Bootstrapped software-team repository support: Project Memory, OKF document bundles, GitHub Project link, labels, pointers, merge rules, and architecture placeholder checked idempotently.
   ```
...
## 15. Print final checklist
Print a `# Bootstrap checklist` table with columns `Area`, `Result`, and `Evidence`.
Rows: Auth pre-flight; Existing state inspected; Project Memory bundle; Document bundles; `.software-team/`; GitHub Project; Status field and options; Requirement issue handling; Labels; Pointers and merge rules; Architecture placeholder; Log; Non-conformance.
