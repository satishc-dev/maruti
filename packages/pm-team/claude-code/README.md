...
5. **Phase 4:** runs the Scrum cycle loop (cycle = 1..budget). Each cycle: dispatch all `requirements-analyst-N` agents in parallel (via `TaskCreate`), then dispatch `spec-reviewer-1` after they return (fresh / feedback); or dispatch `board-manager-1` alone (approved). Prepend each subagent's schema-d work-log entry to `.scrum/<slug>/agents/<agentId>.md`. Warn the user at cycle `budget-2`, HALT at `budget` with continue/abort/finalize choice. Answer mid-loop status queries by reading the TOP entry of each agent's log.
